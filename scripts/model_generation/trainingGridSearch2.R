...
confusionMatrixList <- list()
  ...
  gbmGrid <-  expand.grid(interaction.depth = c(9,13,17,21), 
                          n.trees = 2000, 
                          shrinkage = seq(0, 0.02,.005)[-1],
                          n.minobsinnode = c(20))
  ...
  for(j in 1:nrow(gbmGrid)){
    ...
    if(j %% 4 == 0){
      save(confusionMatrixList, file=paste0("data/gbm_gridSearch2_",as.numeric(args[1]),".RData"))
    }
    
  }

save.image(file=paste0("data/gbm_gridSearch2_",as.numeric(args[1]),".RData"))


# files <- list.files("data", pattern = "gbm_gridSearch_", full.names = TRUE)
...
#   load(files[f])
#   
#   F1vals[[f]] <- unlist(lapply(confusionMatrixList, function(x) x$byClass['F1']))
#   Accvals[[f]] <- unlist(lapply(confusionMatrixList, function(x) x$overall['Accuracy']))
#   
# }
# 
# gbmGrid$F1 <- NA
# gbmGrid$Acc <- NA
# 
# for(i in 1:length(F1vals[[1]])){
#   gbmGrid$F1[i] <- mean(unlist(lapply(F1vals, "[[", i)))
#   gbmGrid$Acc[i] <- mean(unlist(lapply(Accvals, "[[", i)))
# }
