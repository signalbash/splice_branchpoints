...
for(c in 1:length(chrom)){
  branchpoint_df_s=fread(file=paste0("data/outputs/branchpoint_df_with_seq_r2_", chrom[c],".csv"), 
                         data.table=FALSE)
...
index.pos <- which(branchpoint_df_HCN$set == "HC")
index.neg <- which(branchpoint_df_HCN$set != "HC")

set.seed(1)

index.pos.train <- sample(index.pos, 40000)
index.pos.test <- index.pos[!(index.pos %in% index.pos.train)]

index.neg.test <- sample(index.neg, 10000 * 27)
index.neg.train <- index.neg[!(index.neg %in% index.neg.test)]

message("preprocessed")

save.image("data/Preprocessed_data.RData")
save(filteredDescr, branchpoint_df_HCN,dummies,TF_v, rm, 
     index.pos.train,index.pos.test,index.neg.train,index.neg.test,
     file="data/Preprocessed_data_small.RData")
